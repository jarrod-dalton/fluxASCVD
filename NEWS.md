...
- Updated suggested dependency floors to `fluxCore`, `fluxPrepare`, `fluxForecast`, and `fluxValidation` at `>= 1.10.0`.

## fluxASCVD 1.9.0

- Coordinated ecosystem release alignment to version 1.9.0.
- Updated suggested dependency floors to `fluxCore`, `fluxPrepare`, `fluxForecast`, and `fluxValidation` at `>= 1.9.0`.
...
- Updated suggested dependency floors to `fluxCore`, `fluxPrepare`,
  `fluxForecast`, and `fluxValidation` at `>= 1.7.0`.
- No model-logic changes in this release.

## fluxASCVD 1.5.0

- Coordinated release alignment with flux 1.5.0 dependency floors.

- check() note cleanup in package code (explicit stats::rnorm namespacing).

- Licensing update: switched package license to LGPL-3.

## fluxASCVD 1.4.0

- README refresh: replaced stale end-to-end example calls with a current runnable example centered on `ascvd_make_example_ehr()`.
- Naming/docs consistency pass: aligned references to renamed Core helper APIs where applicable.
...
- Require fluxCore >= 1.3.3 (avoids masking stats::var()).

## fluxASCVD 1.3.10

- Rewrite: replace prior ASCVD vignettes with a fresh V0 that introduces the flux ecosystem first, then builds a toy ASCVD ModelBundle step-by-step (no helper magic).
- Teaching: align V0 with Core contracts (schema requirements, time unit via ctx), clarify PackageProvider usage, and simplify the two-entity simulation section.
...
- Require fluxPrepare >= 1.3.5 for vignette builds (time segmentation helpers).
...
- Change: refresh bookkeeping uses `Entity$meta` when available (bundle runtime metadata, not validated state).
